...
import { clearCompanySliders, filterCompanyDropdownOptions, clearCompanyAll } from '../../../../actions/filterActions'
...
class TableInfo extends Component {
	...
	clearFilters(name, type, content) {
		...
		if(dropdownList.indexOf(name) !== -1) {
			this.props.filterCompanyDropdownOptions(type, content)
			document.getElementById(`${type}-${content}`).checked = false
		}
		else if(sliderList.indexOf(name) !== -1){
			this.props.clearCompanySliders(type)
		}
...
export default connect(mapStateToProps, { clearCompanySliders, filterCompanyDropdownOptions, clearCompanyAll, updateCompanyData })(TableInfo)
